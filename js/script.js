...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let lastCalculatedWorkArea = ''; // --- NUEVA VARIABLE para almacenar el Área de Trabajo calculada ---
    ...
    const updateItemInputVisibility = (itemBlock) => {
        ...
        angularDeductionInputGroup.classList.add('hidden');
        ...
        if (type === 'muro') {
            ...
            // --- Llama a la función para actualizar el resumen dentro de cada segmento de cielo ---
             // Esto se hace para que los resúmenes se actualicen si cambias el tipo de item a cielo
    ...
    const createItemBlock = () => {
        ...
        structureTypeSelect.addEventListener('change', (event) => {
         ...
         const relevantInputs = itemBlock.querySelectorAll(
             ...
             '.item-post-spacing, .item-double-structure, .item-cielo-panel-type, .item-plenum, .item-angular-deduction'
         );
    ...
    const calculateMaterials = () => {
             ...
             try {
                ...
                if (type === 'muro') {
                    ...
                    if (itemSpecificErrors.length === 0 && hasValidSegment) {
                     ...
                     let totalCieloPerimeterForAngular = 0; // Sum of FULL perimeters for angular calculation
                     ...
                     if (segmentBlocks.length === 0) {
                         ...
                         segmentBlocks.forEach((segBlock, index) => {
                              ...
                              // --- CÁLCULO DEL PERÍMETRO COMPLETO POR SEGMENTO ---
                         ...
                         itemValidatedSpecs.totalCieloPerimeterSum = totalCieloPerimeterForAngular; // Now holds sum of full perimeters
                         // --- Guarda el valor del descuento en los specs validados ---
                    ...
                    if (itemSpecificErrors.length === 0 && hasValidSegment) {
                         ...
                         // Angular de Lámina (based on sum of full perimeters of segments MINUS deduction)
                         let angularLaminaFloat = 0;
                         // totalCieloPerimeterForAngular now holds the sum of full perimeters
                         if (totalCieloPerimeterForAngular > 0) {
                             // --- Aplica el descuento antes de dividir ---
                             let adjustedPerimeter = totalCieloPerimeterForAngular - (isNaN(angularDeduction) ? 0 : angularDeduction); // Subtract deduction, default to 0 if NaN
                ...
                for (const material in itemOtherMaterialsFloat) {
                 ...
                 // Continúa con el siguiente ítem si hay un error en este, pero no lo incluyas en los specs calculados.
             }
        ...
        if (currentCalculatedItemsSpecs.length > 0 && Object.keys(finalTotalMaterials).length > 0) {
            ...
            currentCalculatedItemsSpecs.forEach(item => {
                ...
                if (item.type === 'muro') {
                    ...
                    if (item.segments && item.segments.length > 0) {
                        item.segments.forEach(seg => {
                             // Nota: El resumen detallado ya se muestra dentro de cada bloque de segmento en la interfaz.
                             // Aquí mostramos solo las dimensiones del segmento para el reporte.
                        ...
                        if (!isNaN(item.totalCieloArea)) {
                         ...
                         if (!isNaN(item.totalCieloPerimeterSum)) {
                            ...
                            resultsHtml += `<p style="margin-left: 20px;"><strong>Suma Perímetros Segmentos:</strong> ${item.totalCieloPerimeterSum.toFixed(2)} m</p>`;
                         }
                    } else {
                        resultsHtml += `<p style="margin-left: 20px;">- Sin segmentos válidos</p>`;
                    }
           ...
           downloadOptionsDiv.classList.remove('hidden'); // Show download options
            ...
            // --- Almacena el estado completo de los resultados, incluyendo Área de Trabajo ---
            ...
            lastCalculatedWorkArea = currentCalculatedWorkArea; // Almacena el área de trabajo leído
             ...
             console.log("No se calcularon materiales totales positivos después de validar ítems válidos o no hay ítems válidos.");
             // Esto podría pasar si no hay ítems válidos o si los cálculos resultan en 0 para todos los materiales.
             resultsContent.innerHTML = '<p>No se pudieron calcular los materiales con las dimensiones ingresadas. Revisa los valores y si hay errores de validación.</p>';
             downloadOptionsDiv.classList.add('hidden'); // Hide download options
...
const generateExcel = () => {
   ...
   sheetData.push([
       ...
       "Espaciamiento Postes (m)", "Pleno (m)", "Metros Descuento Angular (m)", "Estructura Doble", // <- Nueva columna aquí
       "Suma Perímetros Segmentos (m)", "Ancho Total (muro) (m)", "Área Total (m²)" // Nombres de columnas de totales más claros
   ]);
   // --- FIN ENCABEZADOS ---
   ...
   lastCalculatedItemsSpecs.forEach(item => {
        if (item.type === 'muro') {
            ...
            // Aseguramos que el array tenga el tamaño correcto, llenando las columnas de Cielo con vacío.
            const muroCommonDetails = [
                ...
                '',                          // 8: Pleno (vacío para muro)
                '',                          // 9: Metros Descuento Angular (vacío para muro) - Corresponde a la nueva columna
                item.isDoubleStructure ? 'Sí' : 'No' // 10: Estructura Doble - Índice ahora es 10
                // Nota: Las columnas de Totales (Suma Perímetros Seg, Ancho Total Muro, Área Total) empiezan en el índice 11.
            ];
            ...
            // --- Agrega los Totales del Muro ---
            muroSummaryRow.push(
               '',                                                               // 11: Suma Perímetros Segmentos (vacío para Muro)
               !isNaN(item.totalMuroWidth) ? item.totalMuroWidth.toFixed(2) : '', // 12: Ancho Total (m) del Muro
               !isNaN(item.totalMuroArea) ? item.totalMuroArea.toFixed(2) : ''     // 13: Área Total (m²) del Muro
            );
            // --- Fin Totales Muro ---
            ...
            // Fila que etiqueta la sección de Segmentos
             ...
             muroSegmentsLabelRow.push('', '', ''); // 11, 12, 13
            ...
            // Aseguramos que el array tenga el tamaño correcto, llenando las columnas de Muro con vacío.
            const cieloCommonDetails = [
                ...
                // --- Agrega el valor del descuento angular en la posición correcta (índice 9) ---
                !isNaN(item.angularDeduction) ? item.angularDeduction.toFixed(2) : '', // 9: Metros Descuento Angular
                // --- Fin agregación ---
                ''                           // 10: Estructura Doble (vacío para cielo) - Índice ahora es 10
                 // Nota: Las columnas de Totales (Suma Perímetros Seg, Ancho Total Muro, Área Total) empiezan en el índice 11.
            ];
            ...
            // --- Agrega los Totales del Cielo ---
            cieloSummaryRow.push(
               !isNaN(item.totalCieloPerimeterSum) ? item.totalCieloPerimeterSum.toFixed(2) : '', // 11: Suma Perímetros Segmentos
               '',                                                              // 12: Ancho Total (muro) - Vacío para Cielo
               !isNaN(item.totalCieloArea) ? item.totalCieloArea.toFixed(2) : '' // 13: Área Total (m²) del Cielo
            );
            // --- Fin Totales Cielo ---
            ...
            // Fila que etiqueta la sección de Segmentos
             ...
             cieloSegmentsLabelRow[2] = 'Segmentos:'; // Etiqueta "Segmentos:"
              // Agrega celdas vacías para las columnas de totales
              cieloSegmentsLabelRow.push('', '', ''); // 11, 12, 13
